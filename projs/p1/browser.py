...
class HtmlWindow(PyV8.JSClass):
    ...
    def setTimeout(self, code, interval, lang="JavaScript"):
        ...
        # print 'setTimeout', code
        idx = len(self.timers)-1
        gevent.spawn_later(interval/1000.0, self._exec_timer, idx).join()
        return idx

    def _exec_timer(self, idx):
        # print '__exec_timer => ', idx
        if 0 <= idx < len(self.timers):
            t = self.timers[idx]
            if t:
                # print type(t[1].code)
                self.evalScript(t[1].code)
