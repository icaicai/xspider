...
class Node(PyV8.JSClass):
    ...
    def wrap(doc, obj):
        ...
        if type(obj) == bs4.CData:
            return CDATASection(doc, obj)
        
        if type(obj) == bs4.NavigableString:
            return Text(doc, obj)        
...
class Element(Node):
    ...
    @property
    def parentNode(self):
        # print '--------$$$$$$$$---------', type(self.tag.parent), self.tag.parent.name, '<<<'
    ...
    def insertBefore(self, newChild, refChild):        
        ...
        # print 'insertBefore index =>', index, self.tag.name
        # print 'child', newChild.tag.name #, newChild.tag.parent.name
        if index < 0:
...
class Document(Node):
    ...
    def doctype(self):
        for tag in self.doc:
            if isinstance(tag, bs4.Declaration) and tag.startswith("DOCTYPE"):
    ...
    def createElement(self, tagname):        
        element = DOMImplementation.createHTMLElement(self, bs4.Tag(parent=None, name=tagname))
...
class HTMLDocument(Document):
    ...
    def write(self, html):
        if self._html:
            ...
            for tag in bs4.BeautifulSoup(html).contents:
...
def getDOMImplementation(dom=None, **kwds):
    return DOMImplementation(dom if dom else bs4.BeautifulSoup(), **kwds)
    
def parseString(html, **kwds):
    return DOMImplementation(bs4.BeautifulSoup(html), **kwds)
